...
download the accessions. To follow the BiG-MAP tutorial, please use this 
accession file in the following command:
#+BEGIN_EXAMPLE
...
** 2) BiG-MAP.family.py
The main purpose of this script is to compute GCFs using sequence similarity as sole metric. The sequence similarity program Mash is implemented to compute the GCFs of protein sequences. Additionally, a second round of redundancy filtering can be performed with the use of BiG-SCAPE.

BiG-MAP.family takes as input the output directories of anti- or gutSMASH. Therefore, either one of these tools has to been used to predict gene clusters before running BiG-MAP. An example of a gutSMASH output folder can been observed in the [[https://github.com/medema-group/BiG-MAP/tree/master/example_data][example data]] folder (not meant for running purposes!).
...
** 3) BiG-MAP.map.py
This module is designed to align the WGS (paired or unpaired) reads to the reference representatives in each GCF and HGF using /bowtie2/. The following will be computed: RPKM, coverage, core coverage. The coverage is calculated using /Bedtools/, and the read count values using /Samtools/. The general usage is:
...
To map the 10 samples from Schirmer et al. (2018) to the GCF reference representatives, and correct for the BiG-SCAPE GCFs, run:
