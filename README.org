...
#+BEGIN_EXAMPLE
python3 BiG-MAP.map.py -th 10 -b /mnt/scratch/usr001/results/schirmer_metadata.txt -I1 /mnt/scratch/usr001/fastq/schirmer/*pass_1* -I2 /mnt/scratch/usr001/fastq/schirmer/*pass_2* -O /mnt/scratch/usr001/results/ -F /mnt/scratch/usr001/results/
...
clusters can be obseverd using the /--explore/ flag. Next, the 
...
python3 BiG-MAP.analyse.py --explore --compare inspect -B [biom_file] -T [metagenomic/metatranscriptomic] -M [metagroup] -O [outdir] [Options*]

Example:
python3 BiG-MAP.analyse.py --explore --compare -B /mnt/scratch/usr001/BiG-MAP.map.biom -T metagenomic -M DiseaseStatus -g UC Control -O /mnt/scratch/usr001/

Output: 
explore_heatmap.pdf -> contains the top 20 best covered gene clusters
UCvsControl_fitzig.pdf -> comparison between UC and non-IBD using the fitZIG model
UCvsControl_kw.pdf -> comparison between UC and non-IBD using the Kruskal Wallis model
#+END_EXAMPLE
