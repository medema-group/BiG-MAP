...
** 2) BiG-MAP.family.py
The main purpose of this script is to compute GCFs using sequence similarity as sole metric. The sequence similarity program Mash is implemented to compute the GCFs of protein sequences. Additionally, a second round of redundancy filtering can be performed with the use of BiG-SCAPE. BiG-MAP.family takes as input the output directories of anti- or gutSMASH. Options can be investigated by running the /-h/ flag.
